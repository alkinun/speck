...
from speck.evaluation.serving import ModelRunner, _batch_sizes, _percentile, _speck_identity
...
def test_model_runner_compiles_forward_without_replacing_state_owner(monkeypatch):
    ...
    monkeypatch.setattr("speck.evaluation.serving._parameter_count", lambda value: 7)
    compile_mock = Mock(return_value=compiled)
    monkeypatch.setattr("speck.evaluation.serving.torch.compile", compile_mock)
