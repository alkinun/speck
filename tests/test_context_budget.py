...
experiment = Path(__file__).parents[1] / "research" / "flagship" / "targets" / "shape-a"
...
def test_context_budget_exposes_quadratic_global_attention_cost():
    report = estimate_context_budget(
        target_model(),
        (4_096, 131_072),
        effective_tflops=350,
        gpu_hours=5_000,
        weight_bits=4,
        kv_cache_dtype="int8",
    )
    short, long = report["points"]
    assert report["parameters"] == 1_195_878_432
    assert short["tokens_in_budget"] > 8e11
    assert long["compute_multiple_vs_shortest"] > 2
    assert long["state_by_kind"]["attention_kv"] < 0.8 * 1024**3
    assert long["weights_plus_state_bytes"] < 1.4 * 1024**3
