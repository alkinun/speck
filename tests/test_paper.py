...
def test_paper_program_rejects_ratio_placement_readiness_pin_drift(tmp_path):
    copied = tmp_path / "paper-1"
    shutil.copytree(program, copied)
    path = copied / "experiment_program.json"
    value = deepcopy(json.loads(path.read_text(encoding="utf-8")))
    value["ratio_placement_readiness"]["sha256"] = "0" * 64
    path.write_text(json.dumps(value), encoding="utf-8")

    with pytest.raises(ValueError, match="ratio/placement readiness"):
        validate_paper_program(copied, repository_root=root)


def test_paper_program_rejects_attnres_readiness_pin_drift(tmp_path):
    copied = tmp_path / "paper-1"
    shutil.copytree(program, copied)
    path = copied / "experiment_program.json"
    value = deepcopy(json.loads(path.read_text(encoding="utf-8")))
    value["attnres_readiness"]["sha256"] = "0" * 64
    path.write_text(json.dumps(value), encoding="utf-8")

    with pytest.raises(ValueError, match="AttnRes readiness"):
        validate_paper_program(copied, repository_root=root)


def test_paper_program_rejects_latentmoe_readiness_pin_drift(tmp_path):
    copied = tmp_path / "paper-1"
    shutil.copytree(program, copied)
    path = copied / "experiment_program.json"
    value = deepcopy(json.loads(path.read_text(encoding="utf-8")))
    value["stable_latentmoe_readiness"]["sha256"] = "0" * 64
    path.write_text(json.dumps(value), encoding="utf-8")

    with pytest.raises(ValueError, match="Stable LatentMoE readiness"):
        validate_paper_program(copied, repository_root=root)


def test_paper_program_rejects_interaction_readiness_pin_drift(tmp_path):
    copied = tmp_path / "paper-1"
    shutil.copytree(program, copied)
    path = copied / "experiment_program.json"
    value = deepcopy(json.loads(path.read_text(encoding="utf-8")))
    value["interaction_readiness"]["sha256"] = "0" * 64
    path.write_text(json.dumps(value), encoding="utf-8")
...
def test_paper_program_rejects_systems_cost_readiness_pin_drift(tmp_path):
    copied = tmp_path / "paper-1"
    shutil.copytree(program, copied)
    path = copied / "experiment_program.json"
    value = deepcopy(json.loads(path.read_text(encoding="utf-8")))
    value["systems_cost_readiness"]["sha256"] = "0" * 64
    path.write_text(json.dumps(value), encoding="utf-8")

    with pytest.raises(ValueError, match="systems-cost readiness"):
        validate_paper_program(copied, repository_root=root)


def test_paper_program_rejects_novelty_landscape_pin_drift(tmp_path):
    copied = tmp_path / "paper-1"
    shutil.copytree(program, copied)
    path = copied / "experiment_program.json"
    value = deepcopy(json.loads(path.read_text(encoding="utf-8")))
    value["novelty_landscape"]["sha256"] = "0" * 64
    path.write_text(json.dumps(value), encoding="utf-8")
...
def test_paper_program_rejects_adakv_code_audit_pin_drift(tmp_path):
    copied = tmp_path / "paper-1"
    shutil.copytree(program, copied)
    path = copied / "experiment_program.json"
    value = deepcopy(json.loads(path.read_text(encoding="utf-8")))
    value["adakv_code_audit"]["sha256"] = "0" * 64
    path.write_text(json.dumps(value), encoding="utf-8")
...
def test_paper_program_rejects_headkv_code_audit_pin_drift(tmp_path):
    copied = tmp_path / "paper-1"
    shutil.copytree(program, copied)
    path = copied / "experiment_program.json"
    value = deepcopy(json.loads(path.read_text(encoding="utf-8")))
    value["headkv_code_audit"]["sha256"] = "0" * 64
    path.write_text(json.dumps(value), encoding="utf-8")

    with pytest.raises(ValueError, match="HeadKV code audit"):
        validate_paper_program(copied, repository_root=root)


def test_paper_program_rejects_brief_code_audit_pin_drift(tmp_path):
    copied = tmp_path / "paper-1"
    shutil.copytree(program, copied)
    path = copied / "experiment_program.json"
    value = deepcopy(json.loads(path.read_text(encoding="utf-8")))
    value["brief_code_audit"]["sha256"] = "0" * 64
    path.write_text(json.dumps(value), encoding="utf-8")
...
def test_paper_program_rejects_adaptive_cache_gqa_qualification_pin_drift(tmp_path):
    copied = tmp_path / "paper-1"
    shutil.copytree(program, copied)
    path = copied / "experiment_program.json"
    value = deepcopy(json.loads(path.read_text(encoding="utf-8")))
    value["adaptive_cache_gqa"]["qualification_sha256"] = "0" * 64
    path.write_text(json.dumps(value), encoding="utf-8")
...
def test_paper_program_rejects_adaptive_cache_salience_readiness_pin_drift(tmp_path):
    copied = tmp_path / "paper-1"
    shutil.copytree(program, copied)
    path = copied / "experiment_program.json"
    value = deepcopy(json.loads(path.read_text(encoding="utf-8")))
    value["adaptive_cache_salience_readiness"]["sha256"] = "0" * 64
    path.write_text(json.dumps(value), encoding="utf-8")

    with pytest.raises(ValueError, match="adaptive cache salience readiness"):
        validate_paper_program(copied, repository_root=root)
