...
from speck.provenance.contracts import (
    _probability,
...
def test_checked_architecture_promotion_contract_is_valid():
    ...
    assert report == {
        ...
        "external_model_adapters_qualified": 3,
...
def test_helmet_runtime_entry_partition_is_pinned():
    path = contract / "evaluation_manifest.json"
    value = json.loads(path.read_text(encoding="utf-8"))
    helmet = next(suite for suite in value["external_suites"] if suite["id"] == "helmet")
    helmet["runtime_dependency_audit_sha256"] = "0" * 64

    with pytest.raises(ValueError, match="HELMET runtime dependency evidence"):
        _validate_evaluations(value, "architecture-promotion-v1", root)


def test_helmet_materializer_preflight_is_pinned():
    path = contract / "evaluation_manifest.json"
    value = json.loads(path.read_text(encoding="utf-8"))
    helmet = next(suite for suite in value["external_suites"] if suite["id"] == "helmet")
    helmet["materializer_preflight_sha256"] = "0" * 64

    with pytest.raises(ValueError, match="HELMET materializer preflight evidence"):
        _validate_evaluations(value, "architecture-promotion-v1", root)
...
def test_helmet_trec_rights_decision_is_pinned():
    path = contract / "evaluation_manifest.json"
    value = json.loads(path.read_text(encoding="utf-8"))
    helmet = next(suite for suite in value["external_suites"] if suite["id"] == "helmet")
    helmet["trec_rights_decision_sha256"] = "0" * 64

    with pytest.raises(ValueError, match="HELMET TREC rights evidence"):
        _validate_evaluations(value, "architecture-promotion-v1", root)


def test_helmet_multilexsum_decision_is_pinned():
    path = contract / "evaluation_manifest.json"
    value = json.loads(path.read_text(encoding="utf-8"))
    helmet = next(suite for suite in value["external_suites"] if suite["id"] == "helmet")
    helmet["multilexsum_decision_sha256"] = "0" * 64

    with pytest.raises(ValueError, match="HELMET Multi-LexSum evidence"):
        _validate_evaluations(value, "architecture-promotion-v1", root)


def test_helmet_narrativeqa_decision_is_pinned():
    path = contract / "evaluation_manifest.json"
    value = json.loads(path.read_text(encoding="utf-8"))
    helmet = next(suite for suite in value["external_suites"] if suite["id"] == "helmet")
    helmet["narrativeqa_decision_sha256"] = "0" * 64

    with pytest.raises(ValueError, match="HELMET NarrativeQA evidence"):
        _validate_evaluations(value, "architecture-promotion-v1", root)


def test_helmet_infinitebench_decision_is_pinned():
    path = contract / "evaluation_manifest.json"
    value = json.loads(path.read_text(encoding="utf-8"))
    helmet = next(suite for suite in value["external_suites"] if suite["id"] == "helmet")
    helmet["infinitebench_decision_sha256"] = "0" * 64

    with pytest.raises(ValueError, match="HELMET InfiniteBench evidence"):
        _validate_evaluations(value, "architecture-promotion-v1", root)


def test_helmet_seeded_demo_repair_is_pinned():
    path = contract / "evaluation_manifest.json"
    value = json.loads(path.read_text(encoding="utf-8"))
    helmet = next(suite for suite in value["external_suites"] if suite["id"] == "helmet")
    helmet["seeded_demo_repair_qualification_sha256"] = "0" * 64

    with pytest.raises(ValueError, match="HELMET seeded-demo repair evidence"):
        _validate_evaluations(value, "architecture-promotion-v1", root)
