...
from speck.provenance.catalog import validate_claim_registry, validate_research_catalog
...
def test_supported_claim_requires_finding_and_result_evidence(tmp_path):
    ...
    claim["evidence"] = ["archive/pregrant-history/findings/105_paper_1_paired_proxy_analysis.md"]
    path = tmp_path / "claims.json"
    path.write_text(json.dumps(registry))
...
def test_catalog_rejects_selected_contract_identity_drift(tmp_path):
    catalog = json.loads((ROOT / "research/catalog.json").read_text())
    next(iter(catalog["active_contracts"].values()))["sha256"] = "0" * 64
    path = tmp_path / "catalog.json"
    path.write_text(json.dumps(catalog))
    with pytest.raises(ValueError, match="selected contract identity mismatch"):
        validate_research_catalog(path, repository_root=ROOT)
