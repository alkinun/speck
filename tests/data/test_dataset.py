...
from speck.data import acquisition
...
def test_repository_tree_is_revision_pinned_recursive_and_deterministic():
    ...
    first = acquisition.discover_source_files(source, 17, api)
    second_api = Api()
    second = acquisition.discover_source_files(source, 17, second_api)
    assert first == second
...
def test_repository_tree_verifies_explicit_gzip_jsonl_files():
    ...
    resolved = acquisition.discover_source_files(source, 17, Api())
...
def test_download_uses_revision_pinned_hf_xet(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(acquisition, "hf_hub_download", download)
    destination = tmp_path / "raw" / "shard.parquet"
    acquisition._download_file(
...
def test_stream_reads_needed_columns_and_only_keeps_raw_when_explicit(tmp_path, monkeypatch):
    ...
    rows = list(
        acquisition.iter_source_file_documents(
            source=source,
            revision="abc123",
...
def test_py3langid_detects_long_english_and_chinese_text():
    ...
    assert acquisition._detect_language(english, "py3langid") == "en"
    assert acquisition._detect_language(chinese, "py3langid") == "zh"
...
def test_remote_source_resumes_at_completed_parquet_boundary(tmp_path, monkeypatch):
    ...
    order = acquisition.discover_source_files(source, 7, Api())["files"]
    fixtures = {}
