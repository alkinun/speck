...
| Stage 6 — Final self-distillation | Rejection-sample the promoted RL parent, verify and deduplicate, mix with verified anchor data, then run assistant-masked self-SFT | 100 production hours plus a 30-hour pilot; the verified-sample [dataset builder](training.md#final-self-distillation) feeds the existing SFT trainer; preserve the pre-self-SFT parent and promote only on held-out transfer |
