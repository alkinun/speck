...
def memory_plan(slot_count, caches):
    """Assign each attention slot to a writer or to the closest preceding writer."""
    ...
    for slot in range(slot_count):
        if slot in writers:
            ...
            plan.append(("read", label))
    return plan


def shared_memory_architecture(source, caches, writer_key_value_heads=None):
    ...
    plan = dict(zip(slots, memory_plan(len(slots), caches)))
...
def arm_definitions(caches, mqa_caches):
    arms = [(f"caches-{count}", count, None) for count in caches]
    arms += [(f"caches-{count}-mqa1", count, 1) for count in mqa_caches]
...
def prepare(args):
    ...
    variants = {}
    summary = {}
    for name, caches, kv_heads in arm_definitions(args.caches, args.mqa_caches):
        config, plan = shared_memory_architecture(source, caches, kv_heads)
        with torch.device("meta"):
            model = SpeckForCausalLM(config)
