"""Audit Multi-LexSum component terms and HELMET prompt determinism without payloads."""
...
def validate_volume(path, storage):
    if path.is_symlink() or not path.is_dir() or path.resolve() != path:
        raise ValueError("Multi-LexSum metadata directory must be real and existing")
    mount = json.loads(
        subprocess.run(
    ...
    if (
        ...
        raise ValueError("Multi-LexSum metadata storage identity changed")
...
def download(spec, directory):
    ...
    if "sha256" in spec and (
        ...
        raise ValueError(f"Multi-LexSum metadata changed: {spec['id']}")
    ...
    if canonical_sha256 != spec.get("canonical_visible_text_sha256", canonical_sha256):
        raise ValueError(f"Multi-LexSum visible metadata changed: {spec['id']}")
    for required in spec["required_substrings"]:
        if required not in content and required not in canonical:
            raise ValueError(f"Multi-LexSum metadata assertion changed: {spec['id']}")
...
def unseeded_train_shuffle_count(source):
    ...
    for node in ast.walk(tree):
        ...
        if (
            ...
            and function.attr == "shuffle"
            and isinstance(function.value, ast.Name)
            and function.value.id == "train_data"
        ):
            count += 1
    return count
...
def helmet_analysis(protocol, checkout):
    for spec in protocol["helmet"]["files"]:
        ...
        if not path.is_file() or file_sha256(path) != spec["sha256"]:
            raise ValueError(f"HELMET Multi-LexSum source changed: {spec['path']}")
    entries = 0
    shots = set()
    for relative in ("configs/summ_short.yaml", "configs/summ.yaml"):
        config = yaml.safe_load((checkout / relative).read_text(encoding="utf-8"))
        entries += sum("multi_lexsum" in name for name in config["datasets"].split(","))
        shots.add(config["shots"])
    data_source = (checkout / "data.py").read_text(encoding="utf-8")
    required = [
        'train_data = all_data["train"]',
        'summary/short',
        'train_data.shuffle().select(range(shots))',
    ]
    if any(value not in data_source for value in required):
        raise ValueError("HELMET Multi-LexSum data use changed")
    unseeded = unseeded_train_shuffle_count(data_source)
    if (
        entries != protocol["helmet"]["expected_entries"]
        or shots != {protocol["helmet"]["expected_shots"]}
        or unseeded != 1
    ):
        raise ValueError("HELMET Multi-LexSum prompt contract changed")
    return {
        "entries": entries,
        "shots": next(iter(shots)),
        "training_summary_demos_in_prompt": True,
        "validation_short_summary_is_reference": True,
        "unseeded_train_data_shuffle_calls": unseeded,
...
def prepare(protocol, protocol_path, checkout):
    ...
    if any(directory.iterdir()):
        raise ValueError("Multi-LexSum metadata directory must be empty")
    evidence = [download(spec, directory) for spec in protocol["evidence"]]
    analysis = helmet_analysis(protocol, checkout)
    return {
        "format": "speck_helmet_multilexsum_decision",
        "format_version": 1,
        "status": "metadata_qualified_rights_and_prompt_determinism_blocked",
...
def check(protocol, report_path, checkout):
    ...
    if (
        report.get("format") != "speck_helmet_multilexsum_decision"
        ...
        or report.get("decision", {}).get("prompt_determinism_qualified") is not False
        or report.get("helmet_analysis") != helmet_analysis(protocol, checkout)
    ):
        raise ValueError("Multi-LexSum decision report is invalid")
    expected = {spec["id"]: spec for spec in protocol["evidence"]}
    for artifact in report["evidence"]:
        path = Path(artifact["path"])
        spec = expected[artifact["id"]]
        if not path.is_file():
            raise ValueError("Multi-LexSum retained metadata is missing")
        if "sha256" in spec and (
            path.stat().st_size != spec["bytes"] or file_sha256(path) != spec["sha256"]
        ):
            raise ValueError("Multi-LexSum retained metadata changed")
        if "canonical_visible_text_sha256" in spec:
            ...
            if observed != spec["canonical_visible_text_sha256"]:
                raise ValueError("Multi-LexSum retained visible metadata changed")
    runner = subprocess.run(
        [
            ...
            "show",
            f"{report['runner_revision']}:scripts/helmet_multilexsum_decision.py",
        ],
        check=True,
        capture_output=True,
    ).stdout
    if hashlib.sha256(runner).hexdigest() != report["runner_sha256"]:
        raise ValueError("Multi-LexSum decision runner changed")
    return report
...
def main(argv=None):
    ...
    if protocol.get("format") != "speck_helmet_multilexsum_decision_protocol":
        raise ValueError("Multi-LexSum protocol has the wrong format")
    checkout = args.helmet_checkout.expanduser().resolve()
    if args.check:
        if protocol.get("status") not in {"frozen_unexecuted", "executed_blocked"}:
            raise ValueError("Multi-LexSum protocol has an invalid checked status")
        report = check(protocol, args.output.expanduser().resolve(), checkout)
    else:
        if protocol.get("status") != "frozen_unexecuted":
            raise ValueError("Multi-LexSum protocol must be frozen and unexecuted")
        report = prepare(protocol, protocol_path, checkout)
        atomic_json(args.output, report)
    print(f"HELMET Multi-LexSum: {report['status']}")
