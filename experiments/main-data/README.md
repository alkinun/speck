...
2. Use the [completed practical checks](../corpus-audit/practical-code-checks.json) and
   ...
   [SQL-labelled follow-up](../corpus-audit/sql-cohort-review.json): common full-read coverage is 149 files,
   with 27 unheld files still outside it and 44 records family-held. The latest batch distinguishes
   schemas, notebooks, dumps and fixture roles, including a CQL file under an original SQL label.
   No new or reused verified origins are available for this batch; original labels/weights and holds remain intact.
