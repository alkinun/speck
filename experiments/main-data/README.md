...
The math route keeps FineMath 4+ and UltraData-Math L2 as separate candidates: the reversible FineMath directory view removes 1.67% of its stock as a conservative candidate, while all L2 rows still lack host metadata. A single normalized cross-source link is recorded without automatic removal; Nemotron-CC-Math remains unavailable because its listed LFS hashes are unusable and no shards were acquired.
The external Cagliostro v3 review identifies OpenMathInstruct-2, FineMath's InfiWebMath 3+ subset and SmolTalk as additional candidates not yet registered in this matrix. The [candidate review](../corpus-audit/recipe-review.json) now links bounded viewer diagnostics for OpenMath and FineMath/InfiWebMath; SmolTalk remains a card review. These candidates remain outside retained inventory and every study arm until their source-use, family/contamination, correctness, finite-supply and runtime evidence is added to a future matrix revision. A late math reweighting is a schedule hypothesis, not an additional arm.
