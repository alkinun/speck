...
from speck.evaluation.ruler_source import check as check_source_bundle
...
def repository_revision():
    return git_revision(repository_root())
...
def _load_contract(path, length):
    ...
    source_manifest_path = repository_artifact(data["source_manifest"])
...
def convert_cases(original_path, destination):
    ...
    with (
        Path(original_path).open(encoding="utf-8") as source,
...
def prepare(args):
    ...
    compatibility_patch = (
        repository_root()
        / "archive/pregrant-history/research/architecture-promotion-v1/patches/ruler_qa_required_docs.patch"
    )
    ...
    report = {
        ...
        "runner_source_path": str(Path(__file__).resolve().relative_to(repository_root())),
    }
...
def check(args):
    ...
    generation_source = subprocess.run(
        [
            ...
            str(repository_root()),
            "show",
            f"{report['runner_revision']}:{report.get('runner_source_path', 'scripts/ruler_case_prepare.py')}",
        ],
        check=True,
        capture_output=True,
    ).stdout
    ...
    with tempfile.TemporaryDirectory(prefix="speck-ruler-check-") as temporary:
        staged_ruler, _ = _stage_sources(Path(temporary), generator, source, compatibility_patch)
        executed_code = directory_identity(
            staged_ruler / "scripts",
            include=lambda path: path.suffix in {".py", ".yaml", ".sh"},
        )
    if (
        report["contract"]["generation_spec_sha256"] != generation_contract_identity(contract)
        ...
        or report["generator"]["compatibility_patch"]["sha256"] != file_sha256(compatibility_patch)
        or report["environment"]["uv_lock_sha256"] != file_sha256(repository_root() / "uv.lock")
...
def main(argv=None):
    ...
    print(f"RULER {report['length']}: {report['status']} ({report['case_identity_sha256'][:12]})")
