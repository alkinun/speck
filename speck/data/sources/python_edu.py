...
from speck.data.sources.stack_edu import _fetch_swh_blob
from speck.data.sources.stack_v3 import _secret_counts
from speck.data.sources.stack_v3_refine import _english_prose_result, _sample_partition
...
def validate_python_edu_config(config, *, config_dir=None):
    ...
    if (
        rights["file_license_metadata"] != "absent"
        or rights["authority"] != "manual_review_required"
    ):
    ...
    with (
        ...
        def consume_pending():
            ...
            for row, (status, raw) in zip(
                ...
                _dump_line(
                    attribution, {key: item for key, item in record.items() if key != "text"}
                )
                sampled_bytes += size
                per_repo[repo] += size
        ...
        for row in _metadata_rows(source_path):
            ...
            if row.get("int_score", 0) < config["filters"]["minimum_integer_score"]:
                ...
                continue
            if (
                not config["filters"]["min_file_bytes"]
                <= row.get("length_bytes", 0)
