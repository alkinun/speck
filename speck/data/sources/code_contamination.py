...
def validate_contamination_config(config, *, config_dir=None):
    ...
    for index, benchmark in enumerate(benchmarks):
        ...
        if (
            ...
            or not benchmark_id
            or Path(benchmark_id).name != benchmark_id
        ):
    ...
    if (
        ...
        or any(
            isinstance(x, bool) or not isinstance(x, int) or not 0 <= x < modulus
            for x in remainders
        )
...
def _match_document(text, pattern, tasks, primary_index, sensitivity_index, policy):
    ...
    for reference in candidates:
        for field in tasks[reference]:
            ...
            if len(field_text) >= policy["minimum_exact_field_characters"] and field_text in joined:
                exact.add(reference)
                break
    critical = {
        reference
...
def scan_code_contamination(config, *, restart=False):
    ...
    parent_report_path = _verify(config["input"]["report"], config["input"]["report_sha256"])
    ...
    sensitivity_index = _ngram_index(tasks, config["policy"]["sensitivity_ngram"], config["policy"])
