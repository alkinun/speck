...
from speck.data.sources.stack_v3_refine import _language_identifier, _sample_partition
...
def validate_web_sample_config(config, *, config_dir=None):
    ...
    for name, value in fields.items():
        ...
        if (
            ...
            and not source["metadata_json_column"]
        ):
        ...
        for row_group in row_groups:
            ...
            for _, _, digest, row, text in sorted(candidates):
                ...
                if not filters["min_document_bytes"] <= size <= filters["max_document_bytes"]:
                    ...
                    continue
                alphabetic_ratio = sum(character.isalpha() for character in text) / max(
                    len(text), 1
                )
