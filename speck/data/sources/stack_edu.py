...
def validate_stack_edu_config(config, *, config_dir=None):
    ...
    repo_cap = _integer(filters["max_bytes_per_language_per_repository"], "repository byte cap", 1)
    ...
    letters = _integer(english["minimum_alphabetic_characters"], "minimum alphabetic characters", 1)
        ...
        for item in config["source"]["metadata_files"]:
            ...
            def consume_pending():
                for row, (status, raw) in zip(
                    pending, executor.map(fetch_blob, [x["blob_id"] for x in pending])
                ):
