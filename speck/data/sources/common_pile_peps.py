...
def validate_peps_config(config, *, config_dir=None):
    ...
    if (
        not isinstance(filters["accepted_metadata_license"], str)
        or not filters["accepted_metadata_license"]
    ):
...
def sample_common_pile_peps(config, *, restart=False):
    ...
    with (
        ...
        for line in source:
            ...
            if (
                not config["filters"]["min_document_bytes"]
                <= size
                <= config["filters"]["max_document_bytes"]
            ):
