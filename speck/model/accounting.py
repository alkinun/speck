...
from speck.provenance.repository import repository_artifact
...
def generate_accounting(spec, root):
    ...
    plan_path = repository_artifact(tokenizer["plan"], root)
    ...
    for path, digest in spec["supersedes"]["legacy_shape_a"].values():
        if hashlib.sha256(repository_artifact(path, root).read_bytes()).hexdigest() != digest:
    ...
    if contract_version >= 2:
        for predecessor_name in ("scale_target_spec_v1", "scale_accounting_v1"):
            ...
            if (
                hashlib.sha256(
                    repository_artifact(predecessor["path"], root).read_bytes()
                ).hexdigest()
                != predecessor["sha256"]
            ):
        ...
        if (
            hashlib.sha256(
                repository_artifact(embedding_contract["path"], root).read_bytes()
            ).hexdigest()
...
def load_and_generate(spec_path, root=None):
    ...
    root = Path(root) if root is not None else find_repository_root(spec_path)
