...
from speck.provenance.io import file_sha256 as _file_sha256
from speck.provenance.repository import repository_root as find_repository_root
from speck.provenance.validation import require_keys as _require_keys
...
def _validate_promotion_protocol(path, repository_root, policy_id, tokenizer=None):
    from speck.evaluation.protocols import ANSWER_SETS, RETRIEVAL_TEMPLATES
    ...
    for module in (runner["adaptation_module"], runner["length_evaluation_module"]):
        ...
        if not module_path.is_file():
            module_path = repository_root / module.replace(".", "/") / "__init__.py"
        if not module_path.is_file():
            raise ValueError(
...
def _validate_evaluations(manifest, policy_id, repository_root):
    from speck.evaluation.external import validate_external_suite
...
def validate_research_contract(directory, tokenizer=None):
    ...
    repository_root = find_repository_root(directory)
