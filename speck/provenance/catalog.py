...
def validate_claim_registry(path, *, repository_root=None):
    ...
    for claim in claims:
        ...
        if claim["status"] == "supported" and (
            not any("findings" in Path(value).parts for value in claim["evidence"])
            or not any("results" in Path(value).parts for value in claim["evidence"])
        ):
            raise ValueError(
...
def validate_research_catalog(path, *, repository_root=None):
    ...
    if catalog["format"] != CATALOG_FORMAT or catalog["format_version"] != CATALOG_VERSION:
        raise ValueError("unsupported research catalog format")

    selected = catalog["active_contracts"]
    if not isinstance(selected, dict) or not selected:
        raise ValueError("catalog requires explicitly selected contracts")
    for role, identity in selected.items():
        _exact_keys(identity, {"path", "sha256", "execution"}, f"contract {role}")
        contract_path = _repository_path(root, identity["path"], f"contract {role}", kind="file")
        if file_sha256(contract_path) != identity["sha256"]:
            raise ValueError(f"selected contract identity mismatch: {role}")
    status_path = _repository_path(root, catalog["status_record"], "current status", kind="file")
    status = _load_object(status_path, "current status")
    if not isinstance(status.get("work"), list) or not status["work"]:
        raise ValueError("current status requires work items")
    _unique_ids(status["work"], "status work")
    for work in status["work"]:
        if any(not isinstance(work.get(key), str) or not work[key] for key in ("state", "next")):
            raise ValueError(f"status {work['id']} requires state and next action")
        if "evidence" in work:
            _repository_path(root, work["evidence"], f"status {work['id']} evidence", kind="file")
    ...
    return {
        ...
        "status_record": str(status_path.relative_to(root)),
    }
