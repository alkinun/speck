...
| Common code review | 49 complete texts / 67,461 tokens reviewed; unheld Go/Rust reading complete; 42 of 218 records family-held | [Go/Rust review](experiments/corpus-audit/go-rust-cohort-review.json), [family/provenance follow-up](experiments/corpus-audit/code-family-provenance.json); 129 unheld records unread; no yield estimate or admission |
...
2. **Complete the fixed code-cohort assessment, then compare other banks.** The original
   ...
   sampling frames remain distinct. The [Go/Rust follow-up](experiments/corpus-audit/go-rust-cohort-review.json)
   extends common full-text coverage to 49 records; it adds static findings without new origin checks.
   The [family assessment](experiments/corpus-audit/code-family-provenance.json)
   propagates GoLLIE and vendored Pylint holds across known cohort links: 42 records are now held.
   Installed Arcade/stringutils identities are verified; stringutils also matches its published wheel.
   Continue the 129 currently unheld records outside full-read coverage. Complete broader lineage
