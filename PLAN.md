...
| Common code review | 138 complete texts / 240,912 tokens reviewed; 44 of 218 records family-held; 38 unheld records unread | [Java review](experiments/corpus-audit/java-cohort-review.json); source-notice and caller/test-context questions recorded; all holds preserved; no yield estimate or admission |
...
2. **Complete the fixed code-cohort assessment, then compare other banks.** The original
   ...
   sampling frames remain distinct. The [Java follow-up](experiments/corpus-audit/java-cohort-review.json)
   extends common full-text coverage to 138 records, preserving all 44 holds. A restrictive notice in
   a `no_license` record needs source-use review; caller contracts and teaching/test context remain
   separate from measured behavior. Original source labels and weights remain intact.
   Installed Arcade/stringutils identities are verified; stringutils also matches its published wheel.
   Continue the 38 currently unheld records outside full-read coverage. Complete broader lineage
