...
| R10 | Decide tokenizer D5 | Formal static, corpus exclusion, and fixed-document stream pass; exact fixed-FLOP token stops and a shared 72M-reference-token post-endpoint continuation are frozen; continuation output, run materializer, seven runs, and unopened D5 audit remain | Complete continuation and the seven-run pilot, open the audit once, then freeze artifact/hash and repacking branch before E1/C0 | SPE-117 |
