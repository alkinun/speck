...
| R10 | Decide tokenizer D5 | Formal static, stream, and corrected batch-four preflight gates pass; the three immutable seed-42 screen records are materialized, binding model/backbone, tokenizer, data, evaluation, optimizer, stop, and Git identities; their exact trainer and document-level evaluator remain | Qualify the manifest consumer, run the screen, reproject confirmation from observed overhead, complete eligible runs, open D5 once, then freeze artifact/hash | SPE-117 |
