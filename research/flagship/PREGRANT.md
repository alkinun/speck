...
| R3 | Calibrate production data operations | Full 20B attempt safely paused after four source-file checkpoints; active 2B six-category calibration frozen but blocked until `/mnt/speck-data` is mounted | At least 2B exercises real download, filtering, exact/near dedup, packing, cleanup, resume, and disjointness; measured 20B projection receives an explicit fallback decision or the full run resumes | SPE-173 |
