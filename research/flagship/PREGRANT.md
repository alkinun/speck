...
| R10 | Decide tokenizer D5 | Formal static, corpus exclusion, and fixed stream pass; the first continuation stopped unpublished on science exhaustion, and a total-preserving 72M-token availability-adjusted successor is frozen before model outputs; continuation, run materializer, seven runs, and unopened D5 remain | Complete corrected continuation and the seven-run pilot, open the audit once, then freeze artifact/hash and repacking branch before E1/C0 | SPE-117 |
