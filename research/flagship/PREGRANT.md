...
| R10 | Decide tokenizer D5 | Formal static, stream, and batch-four throughput gates pass; the fixed-FLOP defect was caught before outputs and corrected v2 records now pass independent accounting with unchanged optimizer boundaries; screen execution remains blocked on their consumer | Qualify the exact v2 token-stream/resume consumer and document evaluator, authorize and run the screen, reproject confirmation, complete eligible runs, open D5 once, then freeze artifact/hash | SPE-117 |
