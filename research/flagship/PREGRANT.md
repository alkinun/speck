...
| R9 | Optional FP8 | Complete by conservative fallback: bf16 is frozen for grant 1 and FP8 is deferred; bf16 still requires R7's GH200 execution qualification | Numerical, quality, throughput, memory, and resume gates pass on GH200, or bf16 is frozen | SPE-83 |
| R10 | Decide tokenizer D5 | Corrected v2 records pass independent accounting; all 63 real packed shards, 12,470 evaluation documents, and initial/resume/final cursors pass no-model qualification; screen execution remains blocked on checkpointed trainer and CUDA model/evaluator qualification | Qualify exact-shape CUDA forward/backward, checkpoint/resume, and document NLL; bind execution records, run the screen, reproject confirmation, complete eligible runs, open D5 once, then freeze artifact/hash | SPE-117 |
