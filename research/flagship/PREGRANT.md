...
| R4 | Build the evaluation firewall | Real per-category tokenizer/selection/D5/E2 targets, primary/unseen source roles, calibration-fallback authority, and deterministic globally near-deduplicated input plan frozen; input preparation and partition materialization pending | Tokenizer sample, equal-byte selection set, unseen-source slices, and sealed audits are immutable and decontaminated before outputs | SPE-114 |
