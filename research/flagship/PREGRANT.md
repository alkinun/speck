...
| R10 | Decide tokenizer D5 | Corrected data/cursor gates pass; a pre-retry BF16 policy now qualifies exact-shape eager CUDA checkpoint replay with exact loss/cursor and bounded model/optimizer differences; screen execution remains blocked on compiled training and document NLL | Qualify compiled exact-shape training and CUDA document NLL, bind execution records, run the screen, reproject confirmation, complete eligible runs, open D5 once, then freeze artifact/hash | SPE-117 |
