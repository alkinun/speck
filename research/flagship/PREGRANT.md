...
| R10 | Decide tokenizer D5 | Corrected data and cursor gates pass; exact-shape CUDA checkpoint replay preserves loss and cursor exactly but model/optimizer differ by 2^-14, so the exact gate failed and screen execution remains blocked | Freeze a numerical resume successor before retry, qualify CUDA model/evaluator, bind execution records, run the screen, reproject confirmation, complete eligible runs, open D5 once, then freeze artifact/hash | SPE-117 |
