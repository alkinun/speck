...
| R10 | Decide tokenizer D5 | Formal static, stream, and batch-four throughput gates pass; the first three screen records were rejected before model outputs because v10's fixed-FLOP constant differs from the bound preflight accounting, although corrected optimizer boundaries are unchanged | Freeze the corrected target and v2 manifests, qualify their trainer/evaluator, run the screen, reproject confirmation, complete eligible runs, open D5 once, then freeze artifact/hash | SPE-117 |
